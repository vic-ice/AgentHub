...
> 基准日期：2026-08-27
> 代码基准：分支 `codex/search-deepresearch-quality`，实现提交 `f039f0609f511c7e0a1d48dccb8b1776645dee32`（父基线 `5ebe047`）
...
当前 backend/tests 有 74 个 test_*.py 模块。以实现提交 `f039f06` 的代码执行：

~~~text
cd backend
.venv/Scripts/python.exe -m pytest -q
680 passed, 38 subtests passed
~~~
...
当前项目已经形成一条可清楚解释的主架构：普通 Chat 用 Controller 完成自然语言理解和结构化动作提议；Search 用一次理解、联合检索和一次综合快速增强回答；MemoryWriteGateway 与 ReadingService 分别守住长期记忆和阅读资产边界；RecommendationService 在普通推荐时融合 Memory 与 Shelf；DeepResearchRunner 则在用户显式开启后围绕结论缺口开展独立研究，并通过 PublicationPacket 把模型判断与外部证据无损交给最终编辑模型。数据库、版本链、事件、Trace 和架构测试为这些链路提供可追溯性。
